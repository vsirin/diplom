...
# center of the circumscribed circle of a spherical triangle, vertex of the Voronoi diagram
def center(x, y, z):
    c = np.vstack([x, y, z])
    x0 = np.mean(c, axis=0)
    x0 = x0 / norm(x0)
...
def get_dual(g, points):
    ...
    for v in g.nodes():
        ...
        try: 
            n_cyc = nx.cycle_basis(g.subgraph(neigh))[0]   # adjacent vertex cycle
            n_cyc.append(n_cyc[0])
        except IndexError:
            pass
            #print(nx.cycle_basis(g.subgraph(neigh)))
        
        face = []   # points of the area
...
# minimum of distances between regions at a distance> 3
